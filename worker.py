...
def download(url, post_id, gender):
    url = url
    name = url.split('/')[-1]
    if not os.path.isfile('/var/www/html/images/{}/{}.jpg'.format(gender, name)):
        img_data = requests.get(url).content
        with open('//var/www/html/images/{}/{}'.format(gender, name), 'wb') as handler:
...
def monitor_post(id, next_check, last_sleep):
    ...
    if next_check < time.time():
        ...
        for comment in post['results']['comments']:
            api_create_comment(id, comment['id'], comment['text'], comment['author']['gender_id'], comment['image'], comment['timestamp'])
